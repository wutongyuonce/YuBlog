...
- `plugins/*` 在 Markdown 渲染阶段继续加工内容，例如补阅读时长、标题锚点、外链处理
...
- `src/components/views/*` 负责把内容集合转换成页面结构，例如博客列表、文章详情、项目列表、Friends 列表页，以及 `Insights` 的页面占位组件
...
| `pnpm test:blog-tags` | 验证博客标签汇总与 AND 匹配 |
| `pnpm test:blog-stats` | 验证首页 Blog 字数、日期间隔与中文格式化 |
| `pnpm test:recent-post-date` | 验证首页近期写作日期文案 |
| `pnpm test:progress-stats` | 验证导航栏年积日与进度百分比 |
| `pnpm test:cjk-emphasis` | 验证 CJK 标点旁的 Markdown 强调 |
...
- `remarkCjkFriendly`：让 `**术语（中文）**` 这类紧挨 CJK 标点的强调按 Typora 习惯渲染
- `remarkDirective`
- `remarkDirectiveSugar`
- `remarkMath`
- `remarkReadingTime`：写入 `wordCount`，并在未手动指定时估算 `minutesRead`
...
plugins/index.ts 统一装配插件
        ↓
remark 阶段处理 CJK 强调、指令、数学、字数/阅读时长
        ↓
...
作用：给文章自动写入 `frontmatter.wordCount`，并在未手动指定时写入 `frontmatter.minutesRead`。
...
mdast-util-to-string 提取纯文本
   ↓
countReadableUnits 统计 CJK 字 + 非 CJK 词
   ↓
写回 file.data.astro.frontmatter.wordCount
   ↓
若未指定 minutesRead，按 200 单位/分钟取整，至少 1 分钟
```

#### 使用方式

- `wordCount` 始终由插件写入，供首页 `getBlogStats()` 汇总
- 如果文章里不写 `minutesRead`，则自动计算
- 如果写 `minutesRead: 0` 或 `false`，页面可以隐藏阅读时长
- `minutesRead` 最终由 `RenderPost.astro` 和 `ListView.astro` 消费
...
| `src/content/friends/data.json`                               | 友链卡片数据源，进入 `/friends/` 页面渲染            |
...
| `data.ts`     | 博客、Insights 与 Friends 的过滤、排序、分组，以及首页统计与近期写作 |
| `blog-tag-filter.js` | 博客标签规范化、汇总与 AND 匹配 |
| `blog-stats.js` | 可读字数、日历日跨度与中文数量格式化 |
| `recent-post-date.js` | 首页近期写作的上海时区相对/绝对日期 |
| `progress-stats.js` | 导航栏本地时区年积日与进度百分比 |
| `misc.ts`     | 面板淡入淡出、滚动锁定          |
| `toc.ts`      | TOC 树生成              |
...
- `getPublishedBlogPosts()`：始终排除 draft，供首页统计与近期写作共用
- `getSortedPosts()`：按 `pubDate` 倒序排序
- `getBlogStats()`：已发布文章的篇数、渲染字数合计、首末篇日历日跨度
- `getRecentBlogPosts()`：已发布文章按日期倒序取最近 5 篇
...
- `getSortedFriends('friends')`：按 `order` 与名称排序友链，供 `/friends/` 页面使用
- `getGroupedFriendsByCategory('friends')`：按分类分组友链卡片数据，页面已不再调用
...
首页统计与近期写作走另一条始终排除草稿的链路：

```text
src/content/blogs/**/*.md(x)
        ↓
getPublishedBlogPosts()
        ├── getBlogStats() → HomeHeader.astro
        └── getRecentBlogPosts() → RecentWriting.astro
```

首页这三项功能的展示规则、日期文案与验证命令见 `docs/feature/首页与导航近期功能说明.md`。
...
| 文件                   | 作用                                                 |
| :------------------- | :------------------------------------------------- |
| `Head.astro`         | SEO、JSON-LD、主题初始化、manifest、sitemap |
...
NavBar.astro
    ↓
LogoButton.astro + ProgressStats.astro
    ↓
...
左侧品牌区固定为 `LogoButton` + `ProgressStats`。后者用访问者本地时间显示今年第几天、年度进度和今日进度，每 30 秒刷新一次。
...
| `ProgressStats.astro` | Logo 旁的年积日、年度进度与今日进度 |
...
| `FriendsView.astro` | 渲染 Friends 卡片列表页 |
...
- 调用 `getSortedFriends('friends')` 获取排序后的友链数据
- 用同一网格渲染全部友链卡片
- 用整卡链接渲染头像、站点名、补充说明与描述
- 当头像缺失时，使用站点名称首字母作为回退占位
- 在页面底部挂载 `FriendsApplyPanel.astro`，左侧展示交换标准与 GitHub PR 入口，右侧展示本站参考格式
...
BaseLayout
    ↓
StandardLayout
    ├── head slot：HomeHeader(title + subtitle + Blog 统计 + TinyDesktop)
    └── article slot：
          RenderPage(collectionType='home', id='index')
          RecentWriting（最近 5 篇已发布文章）
```

来读取 `src/content/home/index.md`，并在介绍下方挂近期写作时间线。
...
- 不再提供 `subtitle`、`bgType: snow`、`StandardLayout` 与自定义正文容器
...
- 提供页面级 frontmatter，例如 `title`、`subtitle`、`description`、`bgType`
